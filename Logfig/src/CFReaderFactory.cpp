...
ConfigReader* CFReaderFactory::CreateConfigReader(const std::string &key, ConFigFileType type) noexcept(false)
{
    ...
    if (!reader) {
        ...
        case XML_FILE:
            throw std::runtime_error("Support of ConFigFileType::XML_FILE not implemented");
        }
...
bool CFReaderFactory::DisposeConfigReader(const std::string &key) noexcept(false)
{
    ...
    if (itr != cm_configReaderList.end()) {
        ...
        if (itr->second->GetType() == ConFigFileType::PROPERTY_FILE) {
            ...
            throw std::runtime_error("Could not delete, ConFigFileType::XML_FILE not implemented");
        }
        return true;
    }
    return false;
